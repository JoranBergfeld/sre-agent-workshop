...
- **Find the Bicep deployment** — identifies
  `.github/workflows/deploy-cosmos-rbac-removal-infra.yml` and the exact run
...
- **Read the commit diff** — examines what changed in `scenarios/cosmos-rbac-removal/infra/bicep/modules/identity.bicep`
...
**Current Version:**
```bicep
// in scenarios/cosmos-rbac-removal/infra/bicep/modules/identity.bicep
...
scenarios/cosmos-rbac-removal/infra/bicep/modules/identity.bicep.
...
- You'll see changes to `scenarios/cosmos-rbac-removal/infra/bicep/modules/identity.bicep`
...
Module 5 — 14:32:47
  └─ Validate Cosmos RBAC Removal Infrastructure runs (Bicep syntax)
  └─ You: Manually trigger Deploy Cosmos RBAC Removal Infrastructure
...
| **Deployment** | CI/CD runs automatically post-merge | You trigger Deploy Cosmos RBAC Removal Infrastructure after merging |
