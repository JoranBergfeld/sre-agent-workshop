...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    -u|--machine-user) VM_USER="$2"; shift 2 ;;
    -h|--help)
      echo "Usage: $0 [--resource-group <rg>] [--machine-name <vm>] [--bastion-name <b>] [--local-port <port>] [--machine-user <user>]"
      exit 0
      ;;
...
VM_ID=$(az vm show --resource-group "$RESOURCE_GROUP" --name "$MACHINE_NAME" --query "id" -o tsv) || true
if [ -z "${VM_ID:-}" ]; then
  echo "Unable to resolve VM resource ID." >&2
  exit 1
fi

echo "Starting Bastion RDP tunnel: localhost:$LOCAL_PORT -> $MACHINE_NAME:3389"
