...
# Usage:
#   ./scenarios/arc-disk-pressure/scripts/access/start-bastion-tunnel.sh rg-srelabarcdisk srelabarcdisk-bas srelabarcdisk-vm01 3389 13389 azureuser
#   ./scenarios/arc-disk-pressure/scripts/access/start-bastion-tunnel.sh rg-srelabarcdisk srelabarcdisk-bas srelabarcdisk-vm01 80 18080 azureuser

RESOURCE_GROUP="${1:-rg-srelabarcdisk}"
BASTION_NAME="${2:-srelabarcdisk-bas}"
MACHINE_NAME="${3:-srelabarcdisk-vm01}"
...
VM_ID="$(az vm show --resource-group "$RESOURCE_GROUP" --name "$MACHINE_NAME" --query id -o tsv)"

echo "Opening Bastion tunnel: localhost:${LOCAL_PORT} -> ${MACHINE_NAME}:${RESOURCE_PORT}"
