...
BASTION_NAME="srelabarcdisk-bas"
LOCAL_PORT=18080
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    -h|--help)
      echo "Usage: $0 [--resource-group <rg>] [--machine-name <vm>] [--bastion-name <b>] [--local-port <port>]"
      exit 0
      ;;
...
echo "========================================"
echo "  Arc Workshop Smoke Test"
echo "========================================"

if ! POWER_STATE=$(az vm get-instance-view --resource-group "$RESOURCE_GROUP" --name "$MACHINE_NAME" --query "instanceView.statuses[1].displayStatus" -o tsv); then
...
if ! VM_ID=$(az vm show --resource-group "$RESOURCE_GROUP" --name "$MACHINE_NAME" --query "id" -o tsv); then
