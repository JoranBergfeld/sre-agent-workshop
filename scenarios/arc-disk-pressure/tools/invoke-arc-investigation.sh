...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    -h|--help)
      echo "Usage: $0 [--workspace-id <id>] [--resource-group <rg>] [--machine-name <arm-vm>] [--computer-name <windows-name>]"
      exit 0
      ;;
...
TS=$(date '+%Y%m%d-%H%M%S')
TRACE_PATH="$OUTPUT_DIR/investigation-trace-arc-disk-pressure-${TS}.log"
POSTMORTEM_PATH="$OUTPUT_DIR/postmortem-arc-disk-pressure-${TS}.md"
...
write_stage "Observe" "Received C: disk-pressure alert on Arc machine '$MACHINE_NAME' (Windows computer '$COMPUTER_NAME')."
write_stage "Investigate" "Collecting telemetry from Azure Monitor and Arc host runtime state."
...
[ -z "$requested_subscription_id" ] || [ "$active_subscription_id" = "$requested_subscription_id" ] || { echo "Azure subscription mismatch: requested '$requested_subscription_id', active '$active_subscription_id'. Run: az account set --subscription \"$requested_subscription_id\"" >&2; exit 1; }; echo "Azure subscription: $active_subscription_name ($active_subscription_id)"
  if az monitor log-analytics query -w "$WORKSPACE_ID" --analytics-query "$KQL" -o json >/dev/null 2>&1; then
    ...
    write_stage "Correlate" "No telemetry records returned yet; continuing with Arc host inspection evidence."
  fi
else
...
\`\`\`bash
./scenarios/arc-disk-pressure/tools/invoke-approved-remediation.sh --action cleanup-arc-disk-pressure --resource-group $RESOURCE_GROUP --machine-name $MACHINE_NAME --change-ticket CHG-12345
\`\`\`
EOF
