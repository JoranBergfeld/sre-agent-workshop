...
bash "$SCRIPT_PATH" --resource-group "$RESOURCE_GROUP" --machine-name "$MACHINE_NAME"
...
printf '{"timestamp":"%s","ticket":"%s","action":"%s","resourceGroup":"%s","vmName":"%s","status":"executed"}\n' \
  "$TS" "$CHANGE_TICKET" "$ACTION" "$RESOURCE_GROUP" "$MACHINE_NAME" >> "$OUTPUT_DIR/actions-audit.log"
