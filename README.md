...
| [Arc-Enabled Server Disk Pressure](scenarios/arc-disk-pressure/) | Azure Arc-enabled Servers | Storage capacity | 2 | 45m | intermediate | high | A bounded process fills the C: drive of an evaluation Arc-enabled server to approximately 8% free space while Arc and IIS remain healthy. |
| [Azure Boards Copilot Handover](scenarios/azure-boards-copilot-handover/) | Azure Functions + Service Bus | Service Bus schema drift | 2 | 45m | intermediate | low | Valid unsupported v2 order events accumulate as an active backlog when the Function only handles the supported order event schema. |
