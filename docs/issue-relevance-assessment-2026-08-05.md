...
| [#7](https://github.com/JoranBergfeld/sre-agent-workshop/issues/7) | Every scenario entry guide now displays the cost profile sourced from `scenario.yaml`, while its scenario-specific **dominant cost drivers** guidance is authored in the README. The per-scenario `guide documents its cost profile` cases and unresolved-placeholder test in [`documentation-contract.test.js`](../scripts/scenario-tools/test/documentation-contract.test.js) verify profile agreement and the presence of completed cost guidance. |
